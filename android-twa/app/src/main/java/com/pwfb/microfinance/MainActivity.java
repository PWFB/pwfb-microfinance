...
public class MainActivity extends Activity {
    ...
    private void continueNativeLogin(WebView v,String u){if(nativeLoginRedirected||pendingNativeToken==null||pendingNativeToken.trim().isEmpty())return;Uri x=Uri.parse(u==null?"":u);if(!"pwfb-frontend.onrender.com".equalsIgnoreCase(x.getHost()))return;if(!"/".equals(x.getPath())&&!"/login".equals(x.getPath()))return;String t=pendingNativeToken.replace("\\","\\\\").replace("'","\\'").replace("\n","\\n").replace("\r","\\r");nativeLoginRedirected=true;v.evaluateJavascript("window.localStorage.setItem('token','"+t+"');window.localStorage.setItem('access_token','"+t+"');window.sessionStorage.setItem('token','"+t+"');window.sessionStorage.setItem('access_token','"+t+"');true;",value->{if(webView!=null)webView.loadUrl(DASHBOARD_URL);});}
