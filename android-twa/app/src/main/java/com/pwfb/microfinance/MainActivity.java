...
public class MainActivity extends Activity {
    ...
    private void registerPasskeyOnMainThread(final boolean replaceExisting, final String token) {
        try {
            ...
            new Thread(() -> {
                ...
                } catch (Exception e) { sendNativePasskeyResult(false, e.getMessage() == null ? "Unable to prepare passkey registration." : e.getMessage(), null); }
            }).start();
        } catch (Throwable t) { sendNativePasskeyResult(false, t.getMessage() == null ? "Unable to start native PWFB passkey registration." : t.getMessage(), null); }
    }
    ...
    private void createNativePasskey(JSONObject options, String token) {
        try {
            ...
            credentialManager.createCredentialAsync(this, request, null, ContextCompat.getMainExecutor(this), new CredentialManagerCallback<CreateCredentialResponse, CreateCredentialException>() {
                ...
                @Override public void onError(CreateCredentialException error) { String message = error == null ? "Native passkey registration was cancelled." : error.getMessage(); sendNativePasskeyResult(false, message == null ? "Native passkey registration failed." : message, null); }
            });
        } catch (Exception e) { sendNativePasskeyResult(false, e.getMessage() == null ? "This Android device cannot create a native PWFB passkey." : e.getMessage(), null); }
    }

    private void verifyNativePasskey(String registrationJson, String challenge, String token) {
        try { JSONObject body = new JSONObject(); body.put("credential", new JSONObject(registrationJson)); body.put("challenge", challenge); JSONObject result = postAuthenticated("/auth/passkey/register/verify", body, token); if (!result.optBoolean("verified", false)) throw new Exception(result.optString("message", "PWFB could not verify the native passkey.")); sendNativePasskeyResult(true, result.optString("message", "PWFB passkey registered successfully on this device."), result); }
        catch (Exception e) { sendNativePasskeyResult(false, e.getMessage() == null ? "PWFB could not verify the native passkey." : e.getMessage(), null); }
    }
    ...
    private JSONObject postAuthenticated(String path, JSONObject body, String token) throws Exception {
        ...
        byte[] bytes = body.toString().getBytes(StandardCharsets.UTF_8); try (OutputStream out = c.getOutputStream()) { out.write(bytes); } return readResponse(c);
    }
