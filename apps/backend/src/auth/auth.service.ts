...
export class AuthService {
  ...
  private async enforceConfiguredSuperAdmin(user: any) {
    const configured = String(process.env.SUPER_ADMIN_EMAIL || '').trim().toLowerCase();
    if (configured && String(user?.email || '').trim().toLowerCase() === configured && user.role !== 'SUPER_ADMIN') {
      user = await this.prisma.user.update({ where: { id: user.id }, data: { role: 'SUPER_ADMIN' } });
    }
    return user;
  }
  ...
  async login(dto: LoginDto) {
    ...
    user = await this.enforceConfiguredSuperAdmin(user);
  ...
  googleConfig() {
    ...
    return { client_id: clientId, server_client_id: clientId, android_client_id: androidClientId, android_configured: Boolean(androidClientId) };
  }
  private async ensureGoogleIdentityTable() { if (this.googleIdentityTableReady) return; await this.prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "GoogleIdentity" ("userId" TEXT PRIMARY KEY REFERENCES "User"("id") ON DELETE CASCADE, "googleSub" TEXT NOT NULL UNIQUE, "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)`); this.googleIdentityTableReady = true; }
  async googleLogin(idToken?: string, requestOrigin?: string, requestClientId?: string, expectedNonce?: string) {
    const isAndroid = requestOrigin === 'android-app'; const clientId = process.env.GOOGLE_CLIENT_ID?.trim();
    ...
    const configuredOrigins = [process.env.GOOGLE_ALLOWED_ORIGINS, process.env.WEBAUTHN_ORIGIN].filter(Boolean).flatMap((value) => String(value).split(','));
    const googleAllowed = [...configuredOrigins, 'https://pwfb-frontend.onrender.com', 'android-app'].map((value) => value.trim().replace(/\/$/, '')).filter(Boolean);
    ...
    try {
      const ticket = await this.googleClient.verifyIdToken({ idToken, audience: clientId }); const payload = ticket.getPayload();
  ...
  private async ensureAuthenticatorTable() { if (this.authenticatorTableReady) return; await this.prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "UserAuthenticator" ("userId" TEXT PRIMARY KEY REFERENCES "User"("id") ON DELETE CASCADE, "secretEnc" TEXT NOT NULL, "enabled" BOOLEAN NOT NULL DEFAULT FALSE, "recoveryCodes" TEXT, "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)`); this.authenticatorTableReady = true; }
  ...
  async authenticatorSetup(authUser: any) { const user = await this.prisma.user.findUnique({ where: { id: authUser.sub } }); if (!user) throw new UnauthorizedException('User not found'); await this.ensureAuthenticatorTable(); const existing = await this.prisma.$queryRawUnsafe<Array<{ enabled: boolean }>>(`SELECT "enabled" FROM "UserAuthenticator" WHERE "userId" = $1 LIMIT 1`); if (existing[0]?.enabled) throw new BadRequestException('Authenticator is already enabled'); const secret = this.base32Decode(randomBytes(20).toString('hex')); const encoded = Buffer.from(secret).toString('base64url'); const encrypted = this.encryptAuthenticatorSecret(encoded); await this.prisma.$executeRawUnsafe(`INSERT INTO "UserAuthenticator" ("userId", "secretEnc", "enabled") VALUES ($1, $2, FALSE) ON CONFLICT ("userId") DO UPDATE SET "secretEnc" = EXCLUDED."secretEnc", "enabled" = FALSE, "recoveryCodes" = NULL, "updatedAt" = CURRENT_TIMESTAMP`, user.id, encrypted); return { configured: true, enabled: false, secret: encoded, otpauthUri: this.buildOtpUri(encoded, user.email), message: 'Scan the QR code in your authenticator app or enter the setup key manually, then enter the 6-digit code.' }; }
