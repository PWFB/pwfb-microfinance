...
export class AuthService {
  ...
  async passkeyRegisterOptions(authUser: any) {
    ...
    const options = await generateRegistrationOptions({
      rpName: RP_NAME, rpID: RP_ID, userName: user.email,
  ...
  async passkeyLoginOptions(email: string) {
    const user = await this.prisma.user.findUnique({ where: { email }, include: { passkeys: true } });
    if (!user) throw new UnauthorizedException('No account found for this email');
    if (!user.passkeys.length) throw new BadRequestException('No passkey is registered for this account');
  ...
  async passkeyLoginVerify(response: any) {
    ...
    if (!passkey) throw new UnauthorizedException('Passkey not recognized');
    const challenge = await this.getWebAuthnChallenge(passkey.userId, 'AUTHENTICATION');
    if (!challenge) throw new BadRequestException('Passkey login session expired');
