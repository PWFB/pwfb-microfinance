...
export class BalmzAiService {
  ...
  private async operationalSnapshot() {
    const [customers, staff, loans, savings, transactions, repayments] = await Promise.all([
      this.prisma.customer.count(), this.prisma.staff.count(), this.prisma.loan.count(), this.prisma.savings.count(), this.prisma.transaction.count(), this.prisma.repayment.count(),
    ]);
    const [negativeSavings, negativeLoans, negativeTransactions, negativeRepayments] = await Promise.all([
      this.prisma.savings.count({ where: { amount: { lt: 0 } } }), this.prisma.loan.count({ where: { amount: { lt: 0 } } }), this.prisma.transaction.count({ where: { amount: { lt: 0 } } }), this.prisma.repayment.count({ where: { amount: { lt: 0 } } }),
    ]);
  ...
  private async financialIntegrityAudit(): Promise<{ status: 'healthy' | 'warning' | 'critical'; checks: number; findings: Finding[] }> {
    ...
    const [cashbookNegative, collectionsNegative, payrollItems, periods, transactions, repayments, loans, savingsAccounts, activeStaff] = await Promise.all([
      this.prisma.cashbookEntry.count({ where: { amount: { lt: 0 } } }), this.prisma.dailyCollection.count({ where: { amount: { lt: 0 } } }), this.prisma.payrollItem.findMany({ select: { id: true, basicSalary: true, allowances: true, deductions: true, netSalary: true } }), this.prisma.financialPeriod.findMany({ select: { id: true, name: true, startDate: true, endDate: true } }), this.prisma.transaction.findMany({ select: { id: true, customerId: true, type: true, amount: true, description: true, createdAt: true } }), this.prisma.repayment.findMany({ select: { id: true, loanId: true, amount: true } }), this.prisma.loan.findMany({ select: { id: true, amount: true } }), this.prisma.savings.findMany({ select: { id: true, customerId: true, amount: true, accountType: true, createdAt: true } }), this.prisma.staff.count({ where: { employmentStatus: 'ACTIVE' } }),
    ]);
    ...
    for (const row of transactions) { const fingerprint = [row.customerId, row.type, row.amount, row.description || '', row.createdAt.toISOString()].join('|'); fingerprintCounts.set(fingerprint, (fingerprintCounts.get(fingerprint) || 0) + 1); }
    ...
    const repaymentTotals = new Map<string, number>(); for (const repayment of repayments) repaymentTotals.set(repayment.loanId, (repaymentTotals.get(repayment.loanId) || 0) + repayment.amount);
  ...
  async diagnose() {
    const snapshot = await this.operationalSnapshot(); const audit = await this.financialIntegrityAudit(); const basicFindings: Finding[] = [];
  ...
  async repairCheck() {
    ...
    const repairable = audit.findings.filter((finding) => finding.fixable);
    return { assistant: 'BALMZ AI', status: audit.status, checks: audit.checks, repairableCount: repairable.length, repairable, message: repairable.length ? 'Review these proposed repairs and approve each change explicitly before modifying financial records.' : 'No automatic financial repairs are proposed. Review warnings manually before making any correction.' };
  }

  private extractOpenAiText(payload: any): string { if (typeof payload?.output_text === 'string') return payload.output_text; const parts: string[] = []; for (const item of payload?.output || []) for (const content of item?.content || []) if (typeof content?.text === 'string') parts.push(content.text); return parts.join('\n').trim(); }
  private extractGeminiText(payload: any): string { if (typeof payload?.output_text === 'string') return payload.output_text.trim(); const parts: string[] = []; for (const candidate of payload?.candidates || []) for (const part of candidate?.content?.parts || []) if (typeof part?.text === 'string') parts.push(part.text); for (const item of payload?.output || []) for (const content of item?.content || []) if (typeof content?.text === 'string') parts.push(content.text); for (const step of payload?.steps || []) for (const content of step?.content || []) if (typeof content?.text === 'string') parts.push(content.text); return parts.join('\n').trim(); }
  private cleanGeminiModel(value: string): string { let model = value.trim().replace(/^['"]|['"]$/g, ''); model = model.replace(/^=+/, ''); model = model.replace(/^https?:\/\/[^/]+\/v1beta\//i, ''); model = model.replace(/^.*\/models\//i, ''); model = model.replace(/^models\//i, ''); model = model.replace(/:generateContent$/i, ''); return model.trim(); }
  private sleep(ms: number) { return new Promise((resolve) => setTimeout(resolve, ms)); }
  private isQuotaError(status: number, body: string) { return status === 429 || /insufficient_quota|credit_balance_exhausted|quota exceeded|resource_exhausted|exceeded your current quota/i.test(body); }
  ...
  private providerOrder(): AiProvider[] { const configured = (process.env.BALMZ_AI_PROVIDER || 'auto').trim().toLowerCase(); if (configured === 'gemini') return ['gemini', 'openai']; if (configured === 'openai') return ['openai', 'gemini']; return process.env.GEMINI_API_KEY?.trim() ? ['gemini', 'openai'] : ['openai', 'gemini']; }
  private localDiagnosticsReply(audit: { status: string; checks: number; findings: Finding[] }) { const actionable = audit.findings.filter((finding) => finding.severity !== 'info'); if (!actionable.length) return 'BALMZ AI provider services are temporarily unavailable, but the built-in financial integrity audit is healthy. No financial correction is required.'; const priority = actionable.find((finding) => finding.severity === 'critical') || actionable.find((finding) => finding.severity === 'warning')!; const lines = actionable.slice(0, 6).map((finding, index) => `${index + 1}. ${finding.severity.toUpperCase()}: ${finding.title} — ${finding.detail}`); return `BALMZ AI provider services are temporarily unavailable because the configured AI quotas are exhausted. The PWFB database diagnostics are still available and remain authoritative.\n\nCurrent integrity status: ${audit.status.toUpperCase()} (${audit.checks} checks).\n\nFirst priority: ${priority.title}.\n\nFindings:\n${lines.join('\n')}\n\nNo financial record should be changed automatically. Review and approve any correction explicitly.`; }

  async chat(message: string) {
    const snapshot = await this.operationalSnapshot(); const integrityAudit = await this.financialIntegrityAudit(); const system = `You are BALMZ AI, the internal operations assistant for PWFB Microfinance. You help the Super Admin diagnose application and operational mistakes. Be precise, concise and conservative. NEVER infer a discrepancy from unrelated raw table counts. NEVER say that savings accounts without transactions are missing deposits unless the database explicitly proves it. Use the supplied integrity audit as the authoritative source for findings and priorities. Do not contradict it with arithmetic such as “6 savings accounts minus 3 transactions”. Never invent financial records. Never instruct the system to silently alter financial records. For financial corrections, identify the exact problem and require explicit administrator approval before any change. Current operational snapshot: ${JSON.stringify(snapshot)}. Current financial integrity audit: ${JSON.stringify(integrityAudit)}`;
