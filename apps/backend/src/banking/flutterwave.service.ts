...
export class FlutterwaveService {
  ...
  private apiBaseUrl() { return (process.env.FLUTTERWAVE_API_BASE_URL?.trim() || 'https://developersandbox-api.flutterwave.com').replace(/\/$/, ''); }
  ...
  private async bvnRequest(path: string, init: RequestInit) {
    ...
    try {
      const response = await fetch(`https://api.flutterwave.com/v3${path}`, { ...init, headers, signal: AbortSignal.timeout(15000) }); const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new BadRequestException(String(payload?.error?.message || payload?.message || `Flutterwave BVN request failed with status ${response.status}`));
      return payload as any;
    } catch (error) { if (error instanceof BadRequestException || error instanceof ServiceUnavailableException) throw error; throw new ServiceUnavailableException('Flutterwave BVN service is unavailable'); }
  }

  async initiateBvnVerification(input: { bvn: string; firstName: string; lastName: string; redirectUrl: string }) {
    const value = String(input.bvn || '').replace(/\D/g, ''); const firstName = String(input.firstName || '').trim(); const lastName = String(input.lastName || '').trim(); const redirectUrl = String(input.redirectUrl || '').trim();
    if (!/^\d{11}$/.test(value)) throw new BadRequestException('BVN must be exactly 11 digits');
    if (!firstName || !lastName) throw new BadRequestException('First name and last name are required to request BVN consent');
    if (!redirectUrl) throw new BadRequestException('BVN redirect URL is required');
    const payload = await this.bvnRequest('/bvn/verifications', { method: 'POST', body: JSON.stringify({ bvn: value, firstname: firstName, lastname: lastName, redirect_url: redirectUrl }) });
    const data = payload?.data ?? {}; const reference = String(data.reference ?? '').trim(); if (!reference) throw new ServiceUnavailableException('Flutterwave did not return a BVN verification reference');
    return { verified: false, pending: true, bvn: value, reference, consentUrl: data.url == null ? null : String(data.url), message: String(payload?.message || 'BVN consent initiated') } satisfies BvnVerificationResult;
  }
  ...
  async getBvnVerification(reference: string): Promise<BvnVerificationResult> {
    ...
    const payload = await this.bvnRequest(`/bvn/verifications/${encodeURIComponent(ref)}?include_complete_message=1`, { method: 'GET' });
    const data = payload?.data ?? {}; const bvnData = data?.bvn_data ?? {}; const status = String(data?.status ?? bvnData?.status ?? '').trim().toUpperCase();
    const firstName = String(data?.first_name ?? data?.firstname ?? bvnData?.firstName ?? bvnData?.first_name ?? '').trim();
    const middleName = String(data?.middle_name ?? data?.middlename ?? bvnData?.middleName ?? bvnData?.middle_name ?? '').trim();
    const lastName = String(data?.last_name ?? data?.lastname ?? bvnData?.surname ?? bvnData?.lastName ?? bvnData?.last_name ?? '').trim();
    const fullName = [firstName, middleName, lastName].filter(Boolean).join(' ').trim(); const verified = status === 'COMPLETED' && Boolean(fullName); const pending = !verified && !['FAILED', 'DECLINED', 'REJECTED', 'CANCELLED'].includes(status);
    return { verified, pending, bvn: String(bvnData?.bvn ?? data?.bvn ?? ''), reference: ref, firstName: firstName || null, middleName: middleName || null, lastName: lastName || null, fullName: fullName || undefined, status, message: String(data?.complete_message ?? payload?.message ?? ''), raw: payload };
  }
  ...
  async createCustomer(input: { email: string; firstName: string; lastName: string; phone?: string | null }) {
    const email = String(input.email || '').trim(); const firstName = String(input.firstName || '').trim(); const lastName = String(input.lastName || '').trim(); if (!email) throw new BadRequestException('Customer email is required for Flutterwave'); if (!firstName || !lastName) throw new BadRequestException('Customer first and last names are required for Flutterwave');
  ...
  async nameEnquiry(bankCode: string, accountNumber: string): Promise<FlutterwaveAccountNameResult> { const code = String(bankCode || '').trim(); const number = String(accountNumber || '').replace(/\D/g, ''); if (!code) throw new BadRequestException('Bank code is required'); if (!/^\d{10}$/.test(number)) throw new BadRequestException('Enter a valid 10-digit account number'); const payload = await this.request('/banks/account-resolve', { method: 'POST', body: JSON.stringify({ account: { code, number }, currency: 'NGN' }) }); const data = payload?.data ?? {}; const accountName = String(data.account_name ?? data.accountName ?? '').trim(); if (!accountName) throw new BadRequestException(payload?.message || 'Account name could not be resolved'); return { accountNumber: String(data.account_number ?? data.accountNumber ?? number).replace(/\D/g, ''), accountName, bankCode: String(data.bank_code ?? data.account_bank ?? code) }; }
