...
export class NibssService {
  private config() {
    ...
    if (!clientId || !clientKey) {
      throw new ServiceUnavailableException('NIBSS integration is not configured');
    }
  ...
  async nameEnquiry(bankCode: string, accountNumber: string): Promise<NibssNameEnquiryResult> {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        ClientId: clientId,
        Nonce: nonce,
        Signature: this.signature(nonce, '', clientKey),
      },
  ...
  async transfer(input: {
    ...
    const response = await fetch(baseUrl, {
      ...
      headers: {
        ...
        ClientId: clientId,
        Nonce: nonce,
        Signature: this.signature(nonce, body, clientKey),
      },
      body,
    ...
    if (!response.ok) {
      throw new ServiceUnavailableException(String(payload.message || `NIBSS transfer failed with status ${response.status}`));
    }
