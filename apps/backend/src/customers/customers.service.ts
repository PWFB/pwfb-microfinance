...
export class CustomersService {
  ...
  async create(dto: CreateCustomerDto, authUser: any) {
    ...
    const customerId = await this.generateCustomerId(); const email = await this.generateLoginEmail(dto.firstName, dto.lastName); const temporaryPassword = this.generateTemporaryPassword(); const hashedPassword = await bcrypt.hash(temporaryPassword, 10);
    try {
  ...
  async update(id: string, dto: UpdateCustomerDto) {
    ...
    if (dto.branchId && !(await this.prisma.branch.findUnique({ where: { id: dto.branchId } }))) throw new BadRequestException('Branch not found');
    if (dto.assignedStaffId && !(await this.prisma.staff.findUnique({ where: { id: dto.assignedStaffId } }))) throw new BadRequestException('Assigned staff not found');
    if (dto.groupId && !(await this.prisma.clientGroup.findUnique({ where: { id: dto.groupId } }))) throw new BadRequestException('Client group not found');
    const firstName = dto.firstName ?? customer.firstName; const middleName = dto.middleName !== undefined ? (dto.middleName.trim() || null) : customer.middleName; const lastName = dto.lastName ?? customer.lastName; const email = dto.email ?? customer.email; const phone = dto.phone ?? customer.phone;
    const updated = await this.prisma.$transaction(async (tx) => { const updatedCustomer = await tx.customer.update({ where: { id }, data: { firstName, middleName, lastName, email, phone, address: dto.address, dateOfBirth: dto.dateOfBirth ? new Date(dto.dateOfBirth) : undefined, branchId: dto.branchId, assignedStaffId: dto.assignedStaffId, groupId: dto.groupId } }); if (customer.userId) await tx.user.update({ where: { id: customer.userId }, data: { ...(email ? { email } : {}), firstName, lastName, phone } }); return updatedCustomer; });
    return { message: 'Customer profile updated successfully', customer: updated };
  }

  async resetPassword(id: string) { const customer = await this.prisma.customer.findUnique({ where: { id }, include: { user: true } }); if (!customer?.user) throw new NotFoundException('Customer login account not found'); const temporaryPassword = `PWFB-${randomBytes(5).toString('hex').toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}`; await this.prisma.user.update({ where: { id: customer.user.id }, data: { password: await bcrypt.hash(temporaryPassword, 10) } }); return { message: 'Customer password reset successfully', customerId: customer.id, loginEmail: customer.user.email, temporaryPassword, note: 'For security, PWFB never stores or displays the previous plaintext password.' }; }

  async overrideIdentity(customerId: string, type: string, value: string, reason: string, adminUser: any) { const identityType = String(type || '').toUpperCase(); if (!['BVN', 'NIN'].includes(identityType)) throw new BadRequestException('Identity type must be BVN or NIN'); const digits = String(value || '').replace(/\D/g, ''); if (digits.length !== 11) throw new BadRequestException(`${identityType} must contain exactly 11 digits`); if (!reason?.trim() || reason.trim().length < 5) throw new BadRequestException('A clear override reason is required'); if (!adminUser?.id) throw new UnauthorizedException('Authenticated Super Admin required'); if (!(await this.prisma.customer.findUnique({ where: { id: customerId }, select: { id: true } }))) throw new NotFoundException('Customer not found'); const existingRows = await this.prisma.$queryRawUnsafe<any[]>(`SELECT id, bvn_status AS "bvnStatus", nin_status AS "ninStatus" FROM customer_identity_verifications WHERE customer_id = $1`, customerId); const existing = existingRows[0]; const verificationId = existing?.id || randomUUID(); const previousStatus = identityType === 'BVN' ? (existing?.bvnStatus || 'NOT_VERIFIED') : (existing?.ninStatus || 'NOT_VERIFIED'); const column = identityType === 'BVN' ? 'bvn' : 'nin'; const statusColumn = identityType === 'BVN' ? 'bvn_status' : 'nin_status'; const atColumn = identityType === 'BVN' ? 'bvn_overridden_at' : 'nin_overridden_at'; const byColumn = identityType === 'BVN' ? 'bvn_override_by_user_id' : 'nin_override_by_user_id'; const emailColumn = identityType === 'BVN' ? 'bvn_override_by_email' : 'nin_override_by_email'; const reasonColumn = identityType === 'BVN' ? 'bvn_override_reason' : 'nin_override_reason'; if (existing) await this.prisma.$executeRawUnsafe(`UPDATE customer_identity_verifications SET ${column} = $1, ${statusColumn} = 'OVERRIDDEN', ${atColumn} = NOW(), ${byColumn} = $2, ${emailColumn} = $3, ${reasonColumn} = $4, updated_at = NOW() WHERE customer_id = $5`, digits, adminUser.id, adminUser.email || null, reason.trim(), customerId); else await this.prisma.$executeRawUnsafe(`INSERT INTO customer_identity_verifications (id, customer_id, ${column}, ${statusColumn}, ${atColumn}, ${byColumn}, ${emailColumn}, ${reasonColumn}) VALUES ($1, $2, $3, 'OVERRIDDEN', NOW(), $4, $5, $6)`, verificationId, customerId, digits, adminUser.id, adminUser.email || null, reason.trim()); await this.prisma.$executeRawUnsafe(`INSERT INTO customer_identity_override_audits (id, verification_id, identity_type, previous_status, new_status, reason, admin_user_id, admin_email) VALUES ($1, $2, $3, $4, 'OVERRIDDEN', $5, $6, $7)`, randomUUID(), verificationId, identityType, previousStatus, reason.trim(), adminUser.id, adminUser.email || null); return { message: `${identityType} override recorded`, customerId, type: identityType, status: 'OVERRIDDEN', maskedValue: this.maskIdentity(digits), reason: reason.trim(), overriddenBy: adminUser.email || adminUser.id }; }
