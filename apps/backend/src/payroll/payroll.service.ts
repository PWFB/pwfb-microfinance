...
export class PayrollService {
  ...
  async create(data: { periodId: string; branchId?: string }) {
    ...
    const staff = await this.prisma.staff.findMany({
      where: { employmentStatus: 'ACTIVE', ...(data.branchId ? { branchId: data.branchId } : {}) },
    ...
    return this.prisma.payroll.create({
      data: {
        ...
        status: 'DRAFT',
        items: {
          create: staff.map((member) => ({ staffId: member.id, basicSalary: 0, allowances: 0, deductions: 0, netSalary: 0 })),
        },
      },
      include: { items: { include: { staff: true } }, period: true, branch: true },
    });
  }
  ...
  async approve(id: string) { const payroll = await this.findOne(id); if (payroll.status !== 'DRAFT') throw new BadRequestException('Only draft payroll can be approved'); if (payroll.items.length === 0) throw new BadRequestException('Payroll must contain at least one staff item'); return this.prisma.payroll.update({ where: { id }, data: { status: 'APPROVED' }, include: { items: true, period: true, branch: true } }); }
  async markPaid(id: string) { const payroll = await this.findOne(id); if (payroll.status !== 'APPROVED') throw new BadRequestException('Only approved payroll can be marked as paid'); return this.prisma.payroll.update({ where: { id }, data: { status: 'PAID', paymentDate: new Date() }, include: { items: true, period: true, branch: true } }); }
