...
export class StaffService {
  ...
  private async ensureBvnVerificationTable() {
    await this.prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "StaffBvnVerification" ("id" TEXT PRIMARY KEY,"reference" TEXT NOT NULL UNIQUE,"bvn" TEXT NOT NULL,"requestedFirstName" TEXT NOT NULL,"requestedLastName" TEXT NOT NULL,"firstName" TEXT,"middleName" TEXT,"lastName" TEXT,"fullName" TEXT,"status" TEXT NOT NULL DEFAULT 'PENDING',"message" TEXT,"createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,"updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,"verifiedAt" TIMESTAMP(3))`);
    await this.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "StaffBvnVerification_bvn_idx" ON "StaffBvnVerification" ("bvn")`);
  }
  ...
  async initiateBvnVerification(input: { bvn: string; firstName: string; lastName: string; redirectUrl: string }) {
    ...
    await this.prisma.$executeRawUnsafe(`INSERT INTO "StaffBvnVerification" ("id","reference","bvn","requestedFirstName","requestedLastName","status","message") VALUES ($1,$2,$3,$4,$5,'PENDING',$6) ON CONFLICT ("reference") DO UPDATE SET "bvn"=EXCLUDED."bvn","requestedFirstName"=EXCLUDED."requestedFirstName","requestedLastName"=EXCLUDED."requestedLastName","status"='PENDING',"message"=EXCLUDED."message","updatedAt"=CURRENT_TIMESTAMP`, randomUUID(), result.reference, result.bvn, input.firstName.trim(), input.lastName.trim(), result.message ?? 'BVN consent initiated');
    return result;
  }
  ...
  async getBvnVerification(reference: string) {
    ...
    await this.ensureBvnVerificationTable();
    const rows = await this.prisma.$queryRawUnsafe<any[]>(`SELECT * FROM "StaffBvnVerification" WHERE "reference" = $1 LIMIT 1`, ref); const row = rows[0];
    if (!row) throw new NotFoundException('BVN verification request not found');
    return { verified: row.status === 'COMPLETED', pending: row.status === 'PENDING', bvn: row.bvn, reference: row.reference, firstName: row.firstName, middleName: row.middleName, lastName: row.lastName, fullName: row.fullName, status: row.status, message: row.message || '' };
  }
  ...
  async handleBvnWebhook(body: any, signature?: string) {
    ...
    if (event && event !== 'bvn.completed') return { ok: true, ignored: true, event };
    ...
    await this.ensureBvnVerificationTable(); if (!reference && !bvn) throw new BadRequestException('Flutterwave BVN webhook did not contain a verification reference or BVN');
    const rows = reference ? await this.prisma.$queryRawUnsafe<any[]>(`SELECT * FROM "StaffBvnVerification" WHERE "reference" = $1 LIMIT 1`, reference) : await this.prisma.$queryRawUnsafe<any[]>(`SELECT * FROM "StaffBvnVerification" WHERE "bvn" = $1 ORDER BY "createdAt" DESC LIMIT 1`, bvn); const row = rows[0];
    if (!row) return { ok: true, ignored: true, reason: 'no-matching-pending-request', reference: reference || null };
    ...
    await this.prisma.$executeRawUnsafe(`UPDATE "StaffBvnVerification" SET "bvn"=COALESCE(NULLIF($1,''),"bvn"),"firstName"=$2,"middleName"=$3,"lastName"=$4,"fullName"=$5,"status"=$6,"message"=$7,"updatedAt"=CURRENT_TIMESTAMP,"verifiedAt"=CASE WHEN $6='COMPLETED' THEN CURRENT_TIMESTAMP ELSE "verifiedAt" END WHERE "id"=$8`, bvn, firstName || null, middleName || null, lastName || null, fullName || null, finalStatus, message || null, row.id);
