...
export default function CustomerAccountPage() {
  ...
  if (!user) return <main className="min-h-screen bg-slate-50 p-6"><p>Please sign in first.</p><Link className="text-emerald-700 font-semibold" href="/login">Login</Link></main>;
      ...
      <div className="mx-auto max-w-3xl space-y-4 p-5">
        ...
        <section className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-500">Email</p><p className="mt-1 break-all font-semibold text-slate-800">{user?.email || customer?.email || "—"}</p></div>
            <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-500">Phone</p><p className="mt-1 font-semibold text-slate-800">{customer?.phone || user?.phone || "—"}</p></div>
