...
function extractVerifiedAccountName(result: any): string {
  const candidates = [
    result?.accountName,
    result?.account_name,
...
export default function CustomerBankTransferPage() {
  ...
  useEffect(() => {
    ...
    setVerifying(true);
    setMessage("");
    pwfbApi.banking.accountName(bankCode, accountNumber).then((result: any) => {
      if (cancelled) return;
      const name = extractVerifiedAccountName(result);
      if (!name) throw new Error("The bank did not return a verified account name.");
      setAccountName(name);
      setVerified(Boolean(result?.verified ?? true));
      setMessage(`Bank verified: ${name}`);
    }).catch((error) => {
      if (!cancelled) {
        setAccountName("");
        setVerified(false);
  ...
  async function submit() {
    ...
    if (!verified || !accountName) return setMessage("Verify the destination bank account before continuing.");
