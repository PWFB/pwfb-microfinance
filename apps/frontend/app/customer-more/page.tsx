  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24">
      <div className="mx-auto w-full max-w-2xl px-4 pb-8 pt-5">
        <header className="mb-6 rounded-3xl bg-emerald-700 p-6 text-white shadow-sm">
          ...
          <p className="mt-1 text-sm text-emerald-100">Manage your PWFB account and financial services.</p>
        </header>

        <div className="space-y-6">
          {sections.map(section => (
            <section key={section.title}>
              <h2 className="mb-3 px-1 text-sm font-semibold text-slate-700">{section.title}</h2>
              <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
                {section.items.map((item, index) => (
                  <Link key={item.href} href={item.href} className={`flex items-center gap-3 p-4 transition hover:bg-slate-50 active:bg-slate-100 ${index !== section.items.length - 1 ? "border-b border-slate-100" : ""}`}>
                    ...
                    <span className="min-w-0 flex-1"><span className="block font-semibold text-slate-900">{item.title}</span><span className="mt-0.5 block text-xs leading-5 text-slate-500">{item.description}</span></span>
          ...
          <section>
            <h2 className="mb-3 px-1 text-sm font-semibold text-slate-700">Session</h2>
