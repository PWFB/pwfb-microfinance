...
export default function CustomerWithdrawPage() {
  ...
  useEffect(() => {
    ...
    if (!bank?.code) {
      setAccountLookupMessage("This bank has no NIBSS institution code configured yet.");
      setAccountName("");
      return;
    }
    let cancelled = false;
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        if (!verifiedName) {
          setAccountName("");
          setAccountLookupMessage("Account name could not be verified. Check the bank and account number.");
          return;
        }
        ...
        if (cancelled) return;
        setAccountName("");
        setAccountLookupMessage(error instanceof Error ? error.message : "Unable to verify this account.");
      } finally {
