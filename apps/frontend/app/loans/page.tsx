...
export default function LoansPage() {
  ...
  const activeLoans = loans.filter(
    (loan) => (loan.status ?? "Pending").toLowerCase() === "active",
  ).length;
    ...
    <main>
      <div className="pwfb-page-header">
        <div>
          ...
          <h1 className="pwfb-page-title">Loans</h1>
          <p className="pwfb-page-description">
            Manage loan applications, balances, interest and repayment status.
          </p>
        </div>

        <Link href="/loans/add" className="pwfb-primary-button">
          + Add Loan
        </Link>
      </div>
      ...
      <section className="pwfb-stat-grid">
        <div className="pwfb-stat-card">
          ...
          <small>Loan accounts</small>
        </div>

        <div className="pwfb-stat-card pwfb-stat-orange">
          <span>Total Loan Value</span>
          <strong>{loading ? "—" : formatAmount(totalAmount)}</strong>
          <small>Principal issued</small>
        </div>
      ...
      <section className="pwfb-panel">
        <div className="pwfb-panel-header">
          <div>
            ...
            <p>All loans currently available in the system.</p>
          </div>

          <span className="pwfb-record-count">
            {loading ? "Loading..." : `${loans.length} records`}
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="pwfb-empty-state">
            ...
            <Link href="/loans/add" className="pwfb-secondary-button">
              Add Loan
            </Link>
          </div>
        ) : (
                ...
                {loans.map((loan) => {
                  ...
                  return (
                    <tr key={loan.id}>
                      <td>
                        <div className="pwfb-customer-cell">
                          ...
                          <div>
                            ...
                            <small>Customer ID: {loan.customerId}</small>
                          </div>
                        </div>
                      </td>

                      <td>{formatAmount(Number(loan.amount || 0))}</td>
                      ...
                      <td>
                        <span className="pwfb-status-badge">{status}</span>
                      </td>
