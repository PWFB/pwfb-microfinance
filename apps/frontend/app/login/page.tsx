...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!data.access_token) { setMessage(data.message || "Login failed"); return; }
      if (remember) localStorage.setItem("token", data.access_token); else sessionStorage.setItem("token", data.access_token);
      ...
      <section className="pwfb-login-phone" aria-label="PWFB login">
        <div className="pwfb-login-inner">
          <header className="pwfb-reference-brand">
            <img src="/pwfb-login-logo.svg" alt="PWFB Perfect Wisdom For Better Ltd" className="pwfb-logo-art" />
          </header>
          ...
          <form onSubmit={handleSubmit} className="pwfb-reference-form">
            ...
            <button type="button" className="pwfb-reference-secondary" onClick={() => setMessage("Google sign-in is not enabled yet.")}><span className="pwfb-google-mark">G</span><span>Continue with Google</span></button>
            <button type="button" className="pwfb-reference-secondary" onClick={() => setMessage("Fingerprint login is not enabled yet.")}><span className="pwfb-fingerprint-mark"><FingerprintIcon /></span><span>Login with Fingerprint</span></button>
          </form>
