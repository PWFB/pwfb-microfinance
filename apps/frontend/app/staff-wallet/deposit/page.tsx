...
export default function StaffWalletDepositPage() {
  const [customerId, setCustomerId] = useState('');
  const [amount, setAmount] = useState('');
  const [description, setDescription] = useState('Staff wallet deposit');
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await apiRequest(`/banking/customers/${encodeURIComponent(customerId.trim())}/deposit`, {
        method: 'POST',
  ...
  return <main className="page"><header><div><small>PWFB STAFF WALLET</small><h1>Deposit</h1><p>Deposit money into a PWFB customer's wallet.</p></div><Link href="/staff-wallet">← Staff Wallet</Link></header>
    <section className="card"><div className="badge">ROLE CONTROLLED</div><form onSubmit={submit}>
