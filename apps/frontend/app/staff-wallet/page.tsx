    ...
    <main className="staff-wallet">
      <aside className={open ? 'sidebar' : 'sidebar collapsed'}>
        ...
        {open && <>
          ...
          <nav>{tools.map(t => <button key={t} onClick={() => alert(`${t} — ${branch}`)}>{t}</button>)}</nav>
        </>}
      </aside>
