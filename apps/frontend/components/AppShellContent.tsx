...
const groups:NavGroup[]=[
 {key:"customers",label:"Customers",icon:"👥",href:"/customers",roles:finance,children:[{label:"Customer Overview",href:"/customers",roles:finance},{label:"Add Customer",href:"/customers/add",roles:finance}]},
 ...
 {key:"organization",label:"Organization",icon:"🏢",href:"/branches",roles:management,children:[{label:"Branches",href:"/branches",roles:management},{label:"Staff Dashboard",href:"/staff-dashboard",roles:allStaff},{label:"Add Staff",href:"/staff/add",roles:["SUPER_ADMIN","ADMIN"]}]},
 ...
 {key:"security",label:"Security",icon:"🔐",href:"/passkey/register",roles:allStaff,children:[{label:"Register your passkey",href:"/passkey/register",roles:allStaff}]},
...
export default function AppShellContent({children}:{children:React.ReactNode}){
 const pathname=usePathname();const router=useRouter();const {user,loading,logout}=useAuth();const[openGroup,setOpenGroup]=useState<string|null>(null);const[mobileOpen,setMobileOpen]=useState(false);const[desktopCollapsed,setDesktopCollapsed]=useState(false);
 const isPublicRoute=publicRoutes.includes(pathname);const isStandaloneDashboard=standaloneDashboardRoutes.includes(pathname);
 ...
 return <div className={`pwfb-shell ${desktopCollapsed?"pwfb-shell-sidebar-collapsed":""}`}>
  {mobileOpen&&<button type="button" aria-label="Close navigation" className="pwfb-sidebar-overlay" onClick={()=>setMobileOpen(false)}/>}<aside className={`pwfb-sidebar ${mobileOpen?"pwfb-sidebar-mobile-open":""} ${desktopCollapsed?"pwfb-sidebar-collapsed":""}`}>
   ...
   <button type="button" className="pwfb-sidebar-collapse-button" aria-label={desktopCollapsed?"Expand sidebar":"Collapse sidebar"} aria-expanded={!desktopCollapsed} onClick={toggleDesktopSidebar} title={desktopCollapsed?"Expand sidebar":"Collapse sidebar"}><span aria-hidden="true">☰</span></button>
   ...
   <nav className="pwfb-nav" aria-label="Main navigation" style={{flex:"1 1 auto",minHeight:0,overflowY:"auto",overflowX:"hidden",paddingBottom:120,WebkitOverflowScrolling:"touch",overscrollBehavior:"contain",scrollbarGutter:"stable"}}>
    <Link href="/dashboard" className={`pwfb-nav-link ${pathname==="/dashboard"?"pwfb-nav-link-active":""}`} title="Dashboard"><span className="pwfb-nav-icon">⌂</span><span>Dashboard</span></Link>
