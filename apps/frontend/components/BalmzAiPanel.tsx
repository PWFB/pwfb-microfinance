...
export default function BalmzAiPanel() {
  ...
  async function send(message = input) {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: 'ai', text: e?.message || 'BALMZ AI could not reach the backend.' }]);
  ...
  async function diagnose() {
    ...
    setBusy(true);
    try {
      const result = await apiRequest('/balmz-ai/diagnose');
      setAudit(result);
  ...
  return <>
    ...
    {open && <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,.28)', display: 'flex', justifyContent: 'flex-end' }} onClick={() => setOpen(false)}>
      <aside onClick={(e) => e.stopPropagation()} style={{ width: 'min(430px, 100vw)', height: '100%', background: '#fff', display: 'flex', flexDirection: 'column', boxShadow: '-12px 0 35px rgba(0,0,0,.18)' }}>
        ...
        <div style={{ margin: '0 12px', padding: '10px 12px', borderRadius: 10, background: statusBackground, color: statusText, display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, fontWeight: 800 }}><span>Financial Integrity</span><span>{statusLabel}{audit?.checks ? ` · ${audit.checks} checks` : ''}</span></div>
